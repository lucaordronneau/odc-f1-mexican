...
class LapTimeInferencePipeline:
    ...
    def compute_driver_statistics(self, driver, grid_position, stint_number, predicted_stint_number, compound, lapnumber, lapnumberbystint, lapnumbertogo, MaxNumber):
    ...
    def predict_lap_time(self, driver, grid_position, predicted_stint_number, compounds, lapnumber_max, lapnumberbystint):
        lap_time_by_stint_result = []
        totallapnumber = 0
        max_tmp = lapnumber_max

        for stint_number in range(1, predicted_stint_number + 1):
            compound = compounds[stint_number-1]
            lapnumberstint = lapnumberbystint[stint_number-1]
            totallapnumber+=lapnumberstint
            lapnumbertogo = lapnumber_max - totallapnumber
            ...
            features = self.compute_driver_statistics(
                driver, grid_position, stint_number, predicted_stint_number, compound, totallapnumber, lapnumberstint, lapnumbertogo, max_tmp
            )
            ...
            # Make prediction
            lap_time_on_stint = self.loaded_model.predict(feature_df_scaled)

            lap_time_by_stint_result.append(lap_time_on_stint)
        return lap_time_by_stint_result